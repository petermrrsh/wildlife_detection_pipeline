...
def parse_args():
    if len(sys.argv) < 2 or len(sys.argv) > 5:
        print("Usage: create_tiles.py <filename> [--tile-size <tile_width> <tile_height>]")
        sys.exit(1)
...
# set the overlap betwen tiles, must be at least double the size of whatever you're trying to identify
overlap_x = 500
overlap_y = 500
...
for y in range(0, height, stride_y):
    for x in range(0, width, stride_x):
        ...
        sub_img.save(f'{tile_dir}/{x}_{y}_tile.jpg')
        
cv2.imwrite(f"{out_dir}/display_img.tif", display_img)